...
model = NestedDiffusionModel(chains)
model.train(n_epochs=30000, batch_size=512)
post = ns.MCMCSamples(model.rvs(1000))
f, a = ns.make_2d_axes(np.arange(5), upper=False, diagonal=False)
a = chains.plot_2d(a, alpha=0.5)
post.plot_2d(a, alpha=0.5)
f.savefig("gaussian.pdf")
